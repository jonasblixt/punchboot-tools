...
#define TEST(name)                                                                            \
    ...
    static struct nala_test_t nala_test_##name = {                                            \
        .name_p = #name, .func = name, .before_fork_func = name##_before_fork, .next_p = NULL \
    };                                                                                        \
...
#define NALA_TEST_FAILURE(message_p) nala_test_failure(__FILE__, __LINE__, message_p)
...
#define NALA_BINARY_ASSERTION(left, right, check, format, formatter)                    \
    do {                                                                                \
        ...
        if (!check(_nala_assert_left, _nala_assert_right)) {                            \
            ...
            NALA_TEST_FAILURE(                                                          \
                formatter(_nala_assert_format, _nala_assert_left, _nala_assert_right)); \
        }                                                                               \
    } while (0);
...
#define NALA_CHECK_EQ(left, right)                                                              \
    _Generic((left),                                                                            \
        char *: _Generic((right),                                                               \
        char *: nala_check_string_equal((char *)(uintptr_t)(left), (char *)(uintptr_t)(right)), \
        const char *: nala_check_string_equal((char *)(uintptr_t)(left),                        \
...
#define NALA_CHECK_NOT_SUBSTRING(left, right) (!nala_check_substring(left, right))

#define NALA_FORMAT_EQ(format, left, right)                                                        \
    _Generic((left),                                                                               \
        char *: _Generic((right),                                                                  \
        char *: nala_format_string(format, (char *)(uintptr_t)(left), (char *)(uintptr_t)(right)), \
        const char *: nala_format_string(                                                          \
                             format, (char *)(uintptr_t)(left), (char *)(uintptr_t)(right)),       \
...
#define ASSERT_EQ(left, right) \
    NALA_BINARY_ASSERTION(left, right, NALA_CHECK_EQ, "%s is not equal to %s.\n", NALA_FORMAT_EQ)

#define ASSERT_NE(left, right) \
    NALA_BINARY_ASSERTION(left, right, NALA_CHECK_NE, "%s is not different from %s.\n", nala_format)

#define ASSERT_LT(left, right) \
    NALA_BINARY_ASSERTION(left, right, NALA_CHECK_LT, "%s is not less than %s.\n", nala_format)

#define ASSERT_LE(left, right) \
    NALA_BINARY_ASSERTION(     \
        left, right, NALA_CHECK_LE, "%s is not less than or equal to %s.\n", nala_format)

#define ASSERT_GT(left, right) \
    NALA_BINARY_ASSERTION(left, right, NALA_CHECK_GT, "%s is not greater than %s.\n", nala_format)

#define ASSERT_GE(left, right) \
    NALA_BINARY_ASSERTION(     \
        left, right, NALA_CHECK_GE, "%s is not greater or equal to %s.\n", nala_format)

#define ASSERT_SUBSTRING(string, substring) \
    NALA_BINARY_ASSERTION(                  \
        string, substring, NALA_CHECK_SUBSTRING, "%s doesn't contain %s.\n", nala_format)

#define ASSERT_NOT_SUBSTRING(string, substring) \
    NALA_BINARY_ASSERTION(                      \
        string, substring, NALA_CHECK_NOT_SUBSTRING, "%s contains %s.\n", nala_format)
...
#define CAPTURE_OUTPUT(stdout_name, stderr_name)                                    \
    ...
    static char *stderr_name = NULL;                                                \
                                                                                    \
    for (stdout_name##i = 0, nala_capture_output_start(&stdout_name, &stderr_name); \
         stdout_name##i < 1;                                                        \
