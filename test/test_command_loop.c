...
static int pb_command_send_result(const struct pb_command_ctx *ctx, struct pb_result *result)
{
...
static int pb_authenticate(struct pb_command_ctx *ctx,
                           ...
                           struct pb_result *result)
{
    struct pb_command_authenticate *auth = (struct pb_command_authenticate *)command->request;

    uint8_t auth_data[1024];

    ctx->authenticated = false;
    printf("Got auth request method: %i, size: %i\n", auth->method, auth->size);
                           ...
                           struct pb_result *result)
{
    ...
    return pb_wire_init_result2(result, PB_RESULT_OK, (char *)version, strlen(version));
}
                        ...
                        struct pb_result *result)
{
    ...
    pb_wire_init_result2(result, PB_RESULT_OK, &board_result, sizeof(board_result));
                           ...
                           struct pb_result *result)
{
    ...
    pb_wire_init_result2(result, PB_RESULT_OK, &status_result, sizeof(status_result));
...
{
    int rc = -PB_RESULT_ERROR;
    struct pb_command_boot_part *boot_command = (struct pb_command_boot_part *)command->request;
                                 ...
                                 struct pb_result *result)
{
    ...
    ssize_t bytes = read(client_fd, stream_buffer[prep_command->id], prep_command->size);
    printf("Done %li\n", bytes);
                              ...
                              struct pb_result *result)
{
    ...
    rc = pb_wire_init_result2(&cmd_result, PB_RESULT_OK, &read_result, sizeof(read_result));
    ...
    printf("  %i\n", rc);
    ssize_t bytes = write(client_fd, tbl, sizeof(struct pb_result_part_table_entry) * c);
...
int test_command_loop_start(struct pb_command_ctx **ctxp)
{
    ...
    pb_command_configure(&ctx, PB_CMD_SLC_SET_CONFIGURATION, &pb_command_config);
    pb_command_configure(&ctx, PB_CMD_SLC_SET_CONFIGURATION_LOCK, &pb_command_config_lock);
    pb_command_configure(&ctx, PB_CMD_SLC_REVOKE_KEY, &pb_revoke_key);

    pb_command_configure(&ctx, PB_CMD_DEVICE_IDENTIFIER_READ, pb_command_device_read_uuid);
    pb_command_configure(&ctx, PB_CMD_AUTHENTICATE, pb_authenticate);

    pb_command_configure(&ctx, PB_CMD_DEVICE_READ_CAPS, pb_read_caps);

    pb_command_configure(&ctx, PB_CMD_AUTH_SET_OTP_PASSWORD, pb_set_otp_password);

    pb_command_configure(&ctx, PB_CMD_BOOTLOADER_VERSION_READ, pb_read_version);

    pb_command_configure(&ctx, PB_CMD_PART_TBL_READ, pb_read_part_table);

    pb_command_configure(&ctx, PB_CMD_PART_TBL_INSTALL, pb_install_part_table);

    pb_command_configure(&ctx, PB_CMD_PART_VERIFY, pb_verify_part);
    ...
    pb_command_configure(&ctx, PB_CMD_STREAM_PREPARE_BUFFER, pb_stream_buffer_prep);

    pb_command_configure(&ctx, PB_CMD_STREAM_WRITE_BUFFER, pb_stream_buffer_write);
