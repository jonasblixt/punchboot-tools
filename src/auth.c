...
static int auth_token(struct pb_context *ctx, const char *token_file_name, uint32_t key_id)
{
    int rc;
...
int action_auth(int argc, char **argv)
{
    ...
    while ((opt = getopt_long(argc, argv, "hvt:d:T:K:P:L:", long_options, &long_index)) != -1) {
        switch (opt) {
        case 'h':
            help_auth();
            return 0;
