...
static int board_command(struct pb_context *ctx, int command, const char *args)
{
    int rc;
    ...
    rc = pb_api_board_command(ctx, command, args, args_len, result_buffer, &result_size);
...
int action_board(int argc, char **argv)
{
    ...
    while ((opt = getopt_long(argc, argv, "hvt:d:sb:A:", long_options, &long_index)) != -1) {
        switch (opt) {
        case 'h':
            help_board();
            return 0;
        ...
        case 'b':
            flag_command = true;
            command = pb_crc32(0, (uint8_t *)optarg, strlen(optarg));
            break;
        case 'A':
